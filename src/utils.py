...
def remove_partial_mean_with_mask(x, node_mask, center_of_mass_mask):
    ...
    # NOTE: N number of points masked for some steps in the hierarchy may be 0. (division by 0.)
    # for those simply replace 0. with 1. -> this won't matter because node_mask will 
    """
    x_masked = x * center_of_mass_mask
    N = center_of_mass_mask.sum(1, keepdims=True) # only one anchor point
    mean = torch.sum(x_masked, dim=1, keepdim=True) / N
    mean[torch.isinf(mean)] = 0. # in some cases division by 0
    mean[torch.isnan(mean)] = 0. # 0 / 0 division just replace it with 0.
    x = x - mean * node_mask
    return x

def remove_partial_mean(x, center_of_mass_mask):
    """
    Subtract center of mass of fragments from coordinates of all atoms
    # NOTE: N number of points masked for some steps in the hierarchy may be 0. (division by 0.)
    # for those simply replace 0. with 1. -> this won't matter because node_mask will 
...
def remove_anchor_mean_with_mask(x, node_mask, anchor_mask):
    """ subtract center of mass of anchor points from all atoms"""
    x_masked = x * anchor_mask
    mean = torch.sum(x_masked, dim=1, keepdim=True)
    x = x - mean * node_mask
    return x
...
def center_of_gravity_zero_gaussian_log_likelihood(x):
    assert len(x.size()) == 3
    B, N, D = x.size()
    assert_mean_zero(x)

    # r is invariant to a basis change in the relevant hyperplance
...
def center_gravity_zero_gaussian_log_likelihood_with_mask(x, node_mask):
    ...
    # out values will have zero contributions
    r2 = sum_except_batch(x.pow(2))
...
def concatenate_features(x, h):
    ...
    if 'integer' in h:
        xh = torch.cat([xh, h['categorical']], dim=2)
    return xh
