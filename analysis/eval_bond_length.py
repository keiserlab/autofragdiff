...
# taken from https://github.com/guanjq/targetdiff/blob/main/utils/evaluation/eval_bond_length.py
...
from analysis import bond_length_config
...
def get_distribution(distances: Sequence[float], bins=bond_length_config.DISTANCE_BINS) -> np.ndarray:
    """ Get teh distribution of distances.
    
    Args:
        distances: (list) List of distances
        bins (list): bins of distances
    Returns:
        np.array: empirical distribution of distances with length equal to DISTANCE_BINS
    """
...
def eval_bond_length_profile(bond_length_profile: BondLengthProfile) -> Dict[str, Optional[float]]:
    # gives the JS divergence of bond distances (different C-(C,O,N) bonds)
    metrics = {}
    for bond_type, gt_distribution in bond_length_config.EMPIRICAL_DISTRIBUTIONS.items():
...
def get_pair_length_profile(pair_lengths):
    ...
    pair_length_profile = {
        'CC_2A': get_distribution(cc_dist, bins=np.linspace(0, 2, 100)), # distances of C-C bonds less than 2 A
        'All_12A': get_distribution(all_dist, bins=np.linspace(0, 12, 100)) # all distances less than 12 A
    }
    return pair_length_profile

def eval_pair_length_profile(pair_length_profile):
    metrics = {}
    for k, gt_distribution in bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items():
...
def plot_distance_hist(pair_length_profile, metrics=None, save_path=None):
    ...
    gt_profile = bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS
    plt.figure(figsize=(6*len(gt_profile), 4))
    for idx, (k, gt_distribution) in enumerate(bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items()):
        plt.subplot(1, len(gt_profile), idx+1)
        x = bond_length_config.PAIR_EMPIRICAL_BINS[k]
...
def bond_distance_from_mol(mol):
    ...
    for bond in mol.GetBonds():
        ...
        bond_type = utils.BOND_TYPES[bond.GetBondType()] 
